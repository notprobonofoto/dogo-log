...
const NewWalksTab = () => {
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-6" role="main" aria-label={t("walks")}>
      <h1 className="text-2xl font-bold text-foreground text-center mb-4">{t("walks")}</h1>
      
      {/* Centered add button */}
      <div className="flex justify-center mb-6">
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-2 px-6 py-3 rounded-full bg-primary text-primary-foreground active:scale-95 transition-all shadow-lg focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 font-semibold"
          aria-label={showForm ? t("cancel") : t("addWalk")}
          aria-expanded={showForm}
        >
          <Plus className="w-5 h-5" aria-hidden="true" />
          {showForm ? t("cancel") : t("addWalk")}
        </button>
      </div>
