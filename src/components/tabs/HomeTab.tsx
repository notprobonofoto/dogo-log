  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-safe">
      <div className="flex justify-between items-center pt-4 mb-2">
        <div />
        <button onClick={() => setShowCode(!showCode)} className="p-2 rounded-full bg-card text-muted-foreground active:scale-95 hover:bg-muted transition-colors">
          <Settings className="w-5 h-5" />
        </button>
      </div>

      {showCode && (
        <div className="animate-fade-in-up bg-card rounded-xl p-4 mb-4 text-center space-y-4">
          <p className="text-sm text-muted-foreground">{t("householdCode")}</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <span className="text-xs">{data.dogs.length} {t("dogs")}</span>
        </button>
      </div>
